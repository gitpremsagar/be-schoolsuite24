...
async function resolveClassSubjects(schoolId, subjects) {
    ...
    for (const item of subjects) {
        ...
        const teacherId = typeof entry.staffProfileId === "string" && entry.staffProfileId
    ...
    const teacherIds = [
        ...new Set([...bySubjectId.values()].filter((id) => id != null)),
    ];
...
export async function createClass(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const { academicYearId, classLevel, section, monthlyFee, classTeacherId, subjects, } = req.body;
...
export async function updateClass(req, res) {
    try {
        ...
        if (body.periodCount !== undefined) {
            if (typeof body.periodCount !== "number" ||
        ...
        if (body.recessAfter !== undefined) {
            ...
            const values = body.recessAfter.filter((n) => typeof n === "number" && Number.isInteger(n));
            if (values.length !== body.recessAfter.length) {
                throw badRequest("recessAfter must be an array of integers");
            }
            const unique = [
                ...new Set(values.filter((n) => n >= 1 && n < nextPeriodCount)),
            ].sort((a, b) => a - b);
            for (const n of values) {
                if (n < 1 || n >= nextPeriodCount) {
                    throw badRequest(`recessAfter values must be between 1 and ${nextPeriodCount - 1}`);
                }
            }
            recessAfter = unique;
        }
...
export async function getClassTimetable(req, res) {
    try {
        ...
        const teacherBySubjectId = new Map(klass.classSubjects.map((cs) => [
            cs.subjectId,
            cs.staffProfile
                ? {
...
export async function setClassTimetable(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const id = param(req, "id");
        ...
        const allowedSubjectIds = new Set(klass.classSubjects.map((cs) => cs.subjectId));
        const seen = new Set();
        const normalized = [];
        for (const slot of slots) {
            if (typeof slot.dayOfWeek !== "number" ||
                ...
                throw badRequest(`dayOfWeek must be an integer between 1 and ${maxDay}`);
            }
            if (typeof slot.periodIndex !== "number" ||
                !Number.isInteger(slot.periodIndex) ||
                slot.periodIndex < 1 ||
                slot.periodIndex > klass.periodCount) {
                throw badRequest(`periodIndex must be an integer between 1 and ${klass.periodCount}`);
            }
            ...
            if (!allowedSubjectIds.has(slot.subjectId)) {
                throw badRequest("One or more subjects are not assigned to this class");
            }
            const key = `${slot.dayOfWeek}:${slot.periodIndex}`;
            if (seen.has(key)) {
                throw badRequest(`Duplicate slot for day ${slot.dayOfWeek} period ${slot.periodIndex}`);
            }
            seen.add(key);
            normalized.push({
                schoolId,
        ...
        const teacherBySubjectId = new Map((refreshed?.classSubjects ?? []).map((cs) => [
            cs.subjectId,
            cs.staffProfile
                ? {
                    id: cs.staffProfile.id,
...
export async function listMyClasses(req, res) {
    try {
        ...
        if (auth.role === "TEACHER") {
            ...
            const primaryByClassId = new Map(homeroomAssignments.map((a) => [a.classId, a.isPrimary]));
            const classesById = new Map(homeroomAssignments.map((a) => [a.class.id, a.class]));
            const missingClassIds = [
                ...new Set(subjectLinks
            ...
            res.json({
                classes: sortClasses([...classesById.values()].map((klass) => ({
                    ...klass,
