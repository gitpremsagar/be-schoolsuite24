...
function isStudentApplicableToEvent(input) {
    if (!input.eventDate || !input.joiningDate)
        return true;
    // Student who joined after the event date is not applicable.
    return input.joiningDate.getTime() <= input.eventDate.getTime();
}
async function loadEventFeeOrThrow(schoolId, id) {
    const event = await prisma.eventFee.findFirst({
        where: { id, schoolId },
        include: eventFeeInclude,
    });
    if (!event)
        throw notFound("Event fee not found");
    return event;
}
async function countApplicableStudents(schoolId, event) {
    const classIds = event.scope === "CLASSES" ? event.classes.map((c) => c.classId) : undefined;
    ...
    return enrollments.filter((e) => isStudentApplicableToEvent({
        joiningDate: e.studentProfile.joiningDate,
        eventDate: event.eventDate,
    })).length;
}
function serializeEvent(event, summary) {
    return {
        id: event.id,
...
export async function listEventFees(req, res) {
    try {
        ...
        const items = await Promise.all(events.map(async (event) => {
            ...
            const accounted = counts.PAID + counts.PARTIAL + counts.UNPAID + counts.WAIVED;
...
export async function createEventFee(req, res) {
    try {
        const auth = getAuthUser(req);
        const schoolId = requireSchoolId(req);
        const { academicYearId, name, description, amount, currency, scope, classIds, eventDate, dueDate, } = req.body;
...
export async function getEventFee(req, res) {
    try {
        const schoolId = requireSchoolId(req);
...
export async function updateEventFee(req, res) {
    try {
        const auth = getAuthUser(req);
        const schoolId = requireSchoolId(req);
        const id = paramId(req.params.id);
        const existing = await loadEventFeeOrThrow(schoolId, id);
        const { name, description, amount, currency, scope, classIds, eventDate, dueDate, isActive, } = req.body;
        const scopeChanging = (scope !== undefined && scope !== existing.scope) ||
        ...
        if (scope !== undefined || classIds !== undefined) {
            ...
            if (nextScope === "SCHOOL") {
                ...
                const incoming = classIds ?? existing.classes.map((c) => c.classId);
                nextClassIds = [
            ...
            const reallyChanging = nextScope !== existing.scope || classesChanging;
            if (reallyChanging || scopeChanging) {
                const paymentCount = await prisma.eventFeePayment.count({
                    where: { eventFeeId: existing.id },
                });
                if (paymentCount > 0) {
                    throw badRequest("Cannot change scope or classes after payments have been recorded");
                }
            }
...
export async function deactivateEventFee(req, res) {
    try {
        const auth = getAuthUser(req);
        const schoolId = requireSchoolId(req);
        const id = paramId(req.params.id);
...
export async function getEventFeeRegister(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const id = paramId(req.params.id);
        const event = await loadEventFeeOrThrow(schoolId, id);
        const classIds = event.scope === "CLASSES"
        ...
        const paymentByStudent = new Map(payments.map((p) => [p.studentProfileId, p]));
        const students = enrollments
            .filter((e) => isStudentApplicableToEvent({
            joiningDate: e.studentProfile.joiningDate,
            eventDate: event.eventDate,
        }))
            .sort((a, b) => {
...
export async function upsertEventFeePayment(req, res) {
    try {
        const auth = getAuthUser(req);
        const schoolId = requireSchoolId(req);
        const id = paramId(req.params.id);
        const { studentProfileId, status, amountPaid, paidAt, notes, } = req.body;
        if (!studentProfileId)
            throw badRequest("studentProfileId is required");
        if (status !== "PAID" &&
            status !== "PARTIAL" &&
            status !== "UNPAID" &&
            status !== "WAIVED") {
...
//# sourceMappingURL=event-fee.controller.js.map