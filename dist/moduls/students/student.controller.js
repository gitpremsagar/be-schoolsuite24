...
function parseStudyingFlag(value) {
...
async function createOneStudent(schoolId, input) {
    const { email, password, name, phone, admissionNumber, rollNumber, dateOfBirth, gender, fatherName, motherName, guardianName, guardianPhone, guardianEmail, permanentAddress, currentAddress, address, bloodGroup, joiningDate, leavingDate, isCurrentlyStudying, classId: classIdInput, academicYearId: academicYearIdInput, academicYear, classLevel, section, } = input;
    if (!email?.trim() || !password || !name?.trim() || !admissionNumber?.trim()) {
        throw badRequest("email, password, name, and admissionNumber are required");
    }
    if (password.length < 8) {
        throw badRequest("Password must be at least 8 characters");
    }
    const studying = parseStudyingFlag(isCurrentlyStudying);
    const enrollHints = [academicYear, classLevel, section].map((v) => typeof v === "string" ? v.trim() : "");
    const enrollHintCount = enrollHints.filter(Boolean).length;
    if (enrollHintCount > 0 && enrollHintCount < 3) {
        throw badRequest("academicYear, classLevel, and section must be provided together");
    }
    ...
    if (enrollHintCount === 3) {
        ...
        if (!klass) {
            throw badRequest(`Class not found: ${level} - ${sectionValue} (${year.name})`);
        }
        classId = klass.id;
        academicYearId = year.id;
    }
...
export async function listStudents(req, res) {
    try {
        ...
        const wantAll = req.query.all === "1" ||
            req.query.all === "true";
        let academicYearId = typeof req.query.academicYearId === "string" && req.query.academicYearId.trim()
...
export async function createStudent(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const result = await createOneStudent(schoolId, req.body);
...
export async function bulkCreateStudents(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const { students, rowOffset } = req.body;
        ...
        const baseRow = typeof rowOffset === "number" &&
        ...
        for (let i = 0; i < students.length; i++) {
            ...
            const input = row && typeof row === "object"
                ? row
                : {};
...
export async function enrollStudent(req, res) {
    try {
        ...
        const { classId, academicYearId, rollNumber } = req.body;
...
export async function updateStudent(req, res) {
    try {
        const schoolId = requireSchoolId(req);
        const id = param(req, "id");
        const { email, password, name, phone, admissionNumber, rollNumber, fatherName, motherName, guardianName, guardianPhone, guardianEmail, permanentAddress, currentAddress, address, gender, bloodGroup, dateOfBirth, joiningDate, leavingDate, isCurrentlyStudying, classId, academicYearId, isActive, } = req.body;
