...
async function requireOwnStaffProfile(req) {
    const auth = getAuthUser(req);
    const staff = await prisma.staffProfile.findUnique({
        where: { userId: auth.userId },
    });
...
function findTodayStaffAttendance(staffProfileId, date) {
    return prisma.staffAttendance.findUnique({
        where: { staffProfileId_date: { staffProfileId, date } },
    });
}
async function createPunchIn(staff, schoolId, method) {
...
async function applyPunchOut(staff, method) {
    const date = toUtcDay(new Date());
    const existing = await findTodayStaffAttendance(staff.id, date);
    if (!existing) {
...
export async function punchIn(req, res) {
    try {
        const auth = getAuthUser(req);
        if (auth.role === "TEACHER") {
            throw badRequest("Teachers must punch in by scanning the school QR code");
        }
        const schoolId = requireSchoolId(req);
...
export async function getStaffPunchQr(req, res) {
    try {
        const schoolId = requireSchoolId(req);
...
export async function rotateStaffPunchQr(req, res) {
    try {
        const schoolId = requireSchoolId(req);
 ...
 */
export async function qrPunch(req, res) {
    try {
        const schoolId = requireSchoolId(req);
