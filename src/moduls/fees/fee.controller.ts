...
export function normalizeFeeAmounts(input: {
  ...
  if (
    (input.status === "PARTIAL" || input.status === "UNPAID") &&
    hint != null &&
...
export async function upsertStudentFeePayment(req: Request, res: Response) {
  try {
    ...
    const feeAmount =
      typeof feeAmountRaw === "number" && !Number.isNaN(feeAmountRaw) && feeAmountRaw >= 0
        ? feeAmountRaw
    ...
    const yearMonths = monthsInRange(
      academicYear.startDate,
      academicYear.endDate,
    );
    const targetOrdinal = year! * 12 + month!;
    const targetInRange = yearMonths.some(
    ...
    const payments = await prisma.$transaction(
      monthsToUpsert.map((m) => {
        ...
        const monthPaidAt =
          monthResolved.status === "UNPAID" ? null : paidAtDate;
    ...
    const payment =
      payments.find((p) => p.year === year && p.month === month) ?? payments[0];
