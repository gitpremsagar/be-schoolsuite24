...
function serializeExamination(
  ...
  counts?: { markSheetCount: number; publishedCount: number; markedCount: number },
) {
...
async function loadExaminationOrThrow(schoolId: string, id: string) {
...
async function getMarkSheetCounts(examinationIds: string[]) {
  if (examinationIds.length === 0) {
    return new Map<
      string,
      { markSheetCount: number; publishedCount: number; markedCount: number }
    >();
  }
  ...
  for (const s of sheets) {
    ...
    if (s.isPublished) entry.publishedCount += 1;
    if (s.marksObtained != null) entry.markedCount += 1;
  }
  return map;
}

async function getTeacherAssignments(schoolId: string, userId: string) {
  const staff = await prisma.staffProfile.findFirst({
    where: { schoolId, userId, staffType: "TEACHER" },
    select: { id: true },
  });
  if (!staff) throw forbidden("Teacher profile required");
...
export async function createExamination(req: Request, res: Response) {
  try {
    ...
    let resolvedScope: ExamScope =
      scope === "SCHOOL" || scope === "CLASSES" ? scope : "CLASSES";

    let teacherKeys: Set<string> | null = null;
    if (auth.role === "TEACHER") {
      ...
      for (const p of papers) {
        if (!teacherKeys.has(`${p.classId}:${p.subjectId}`)) {
          throw forbidden(
            "You can only create exams for your assigned class subjects",
          );
        }
      }
    }
    ...
    if (resolvedScope === "SCHOOL") {
      ...
      for (const classId of paperClassIds) {
        if (!allIds.has(classId)) {
          throw badRequest(
            "One or more paper classes are invalid for this academic year",
          );
        }
      }
...
export async function listExaminations(req: Request, res: Response) {
  try {
    ...
    let where: {
      ...
      OR?: Array<{
        papers: {
          some: { classId: string; subjectId: string };
        };
      }>;
    } = { schoolId };
    ...
    const filtered =
      teacherKeys == null
        ...
        : exams.map((exam) => ({
            ...exam,
            papers: exam.papers.filter((p) =>
              teacherKeys!.has(`${p.classId}:${p.subjectId}`),
            ),
          }));

    const counts = await getMarkSheetCounts(filtered.map((e) => e.id));
    res.json({
      examinations: filtered.map((e) =>
        serializeExamination(e, counts.get(e.id)),
      ),
    });
  } catch (error) {
...
export async function getExamination(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    ...
    if (auth.role === "TEACHER") {
      const teacher = await getTeacherAssignments(schoolId, auth.userId);
      const allowedPapers = exam.papers.filter((p) =>
        teacher.keys.has(`${p.classId}:${p.subjectId}`),
      );
...
export async function updateExamination(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    const id = paramId(req.params.id);
    ...
    if (examDate !== undefined) {
      ...
      if (!parsed) throw badRequest("examDate is required");
      meta.examDate = parsed;
    }

    const syncPapers = papersRaw !== undefined;
    let nextPapers: PaperInput[] | null = null;
    ...
    if (syncPapers) {
      ...
      let resolvedScope: ExamScope = exam.scope;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const nextByKey = new Map(
        nextPapers.map((p) => [`${p.classId}:${p.subjectId}`, p]),
      );

      // Remove papers no longer selected
      const toRemove = exam.papers.filter(
        (p) => !nextByKey.has(`${p.classId}:${p.subjectId}`),
      );
      ...
      for (const p of exam.papers) {
        ...
        if (!next) continue;
        if (next.maxMarks === p.maxMarks) continue;
        ...
        if (over > 0) {
          throw badRequest(
            `Cannot set max marks to ${next.maxMarks}: some students already have higher marks`,
          );
        }
      ...
      const toAdd = nextPapers.filter(
        (p) => !existingByKey.has(`${p.classId}:${p.subjectId}`),
      );
      if (toAdd.length > 0) {
      ...
      const classIdsToAdd = desiredClassIds.filter(
        (cid) => !existingClassIds.has(cid),
      );
      const classIdsToRemove = [...existingClassIds].filter(
        (cid) => !desiredClassIds.includes(cid),
      );
...
export async function deleteExamination(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    const id = paramId(req.params.id);
...
export async function listMarkSheets(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    ...
    const filtered =
      teacherKeys == null
        ? sheets
        : sheets.filter((s) =>
            teacherKeys!.has(`${s.classId}:${s.subjectId}`),
          );
...
export async function saveMarkSheets(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    const examinationId = paramId(req.params.id);
    const { records } = req.body as {
      records?: Array<{ id?: string; marksObtained?: number | null }>;
    };
    ...
    for (const record of records) {
      ...
      if (!paper) throw badRequest("Mark sheet paper not found");

      if (record.marksObtained === null || record.marksObtained === undefined) {
        continue;
      }
      if (
        typeof record.marksObtained !== "number" ||
      ...
      if (record.marksObtained < 0 || record.marksObtained > paper.maxMarks) {
        throw badRequest(
          `marksObtained must be between 0 and ${paper.maxMarks}`,
        );
      }
    }
    ...
    for (let i = 0; i < records.length; i += CHUNK) {
      ...
      await Promise.all(
        chunk.map((record) => {
          const marksObtained =
            record.marksObtained === null || record.marksObtained === undefined
              ? null
...
export async function publishMarkSheets(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    const examinationId = paramId(req.params.id);
    ...
    if (auth.role === "TEACHER") {
      ...
      if (classId && !subjectId) {
        const hasClassAccess = [...teacherKeys].some((key) =>
    ...
    const allowed =
      teacherKeys == null
        ? targets
        : targets.filter((t) =>
            teacherKeys!.has(`${t.classId}:${t.subjectId}`),
          );
...
export async function listMyMarkSheets(req: Request, res: Response) {
  try {
    const auth = getAuthUser(req);
    const schoolId = requireSchoolId(req);
    ...
    const byExam = new Map<
      string,
      {
        ...
        markSheets: Array<{
          id: string;
          subject: { id: string; name: string };
          class: ReturnType<typeof serializeClass>;
    ...
    const exams = [...byExam.values()]
      .map((g) => ({
        ...g,
        markSheets: g.markSheets.sort((a, b) =>
          a.subject.name.localeCompare(b.subject.name),
        ),
      }))
      .sort(
        (a, b) =>
          new Date(b.examination.examDate).getTime() -
