...
import type { StaffType, UserRole } from "@prisma/client";
...
export async function listStaff(req: Request, res: Response) {
  try {
    ...
    const staffType =
      typeof req.query.staffType === "string" ? req.query.staffType : undefined;

    const staff = await prisma.staffProfile.findMany({
      where: {
        schoolId,
        ...(staffType ? { staffType: staffType as StaffType } : {}),
      },
...
export async function createStaff(req: Request, res: Response) {
  try {
    ...
    const {
      ...
      expectedPunchInTime?: string | null;
      expectedPunchOutTime?: string | null;
    };

    if (!email || !password || !name || !staffType || !employeeCode) {
      throw badRequest(
        "email, password, name, staffType, and employeeCode are required",
      );
    }
    ...
    const currentlyWorking =
      typeof isCurrentlyWorking === "boolean" ? isCurrentlyWorking : true;

    const punchInTime = parseOptionalTime(
      expectedPunchInTime,
      "expectedPunchInTime",
    );
    const punchOutTime = parseOptionalTime(
      expectedPunchOutTime,
      "expectedPunchOutTime",
    );
    if (
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const profile = await tx.staffProfile.create({
        data: {
          ...
          leavingDate:
            currentlyWorking || !leavingDate ? null : new Date(leavingDate),
...
export async function updateStaff(req: Request, res: Response) {
  try {
    ...
    if (isProtectedAdminAccount(existing)) {
      if (!adminPassword) {
        throw badRequest(
          "Admin password is required to edit an admin account",
        );
      }
    ...
    const punchInTime =
      expectedPunchInTime !== undefined
        ? parseOptionalTime(expectedPunchInTime, "expectedPunchInTime")
        : undefined;
    const punchOutTime =
      expectedPunchOutTime !== undefined
    ...
    const currentlyWorking =
      typeof isCurrentlyWorking === "boolean"
        ? isCurrentlyWorking
        : undefined;
    ...
    const role: UserRole | undefined =
      staffType === "TEACHER" || staffType === "EMPLOYEE"
        ? staffType
        : undefined;
...
export async function purgeStaff(req: Request, res: Response) {
  try {
    const schoolId = requireSchoolId(req);
    const auth = getAuthUser(req);
    const id = param(req, "id");
