...
export async function getPlatformOverview(req: Request, res: Response) {
  try {
    ...
    const countBySchool = new Map(
      studentCounts.map((row) => [row.schoolId, row._count._all]),
    );
...
export async function getSchoolDetail(req: Request, res: Response) {
  try {
    ...
    res.json({
      ...
      activeEnrollments,
      dueAmount:
        (school.subscription?.pricePerStudent ?? 0) * activeEnrollments,
    });
  } catch (error) {
...
export async function updateSchoolSubscription(req: Request, res: Response) {
  try {
